...
prediction = (π,     y,     z,    q_value, aux_pi) = agent(observations)
target     = (π_hat, y_hat, z_hat) = meta_network(rollouts_of_agents)

q_value = ValueFunction(observations)
target_q_net_out = outputs of target/EMA of the agent network. ??
target_out, _    = unroll_policy_fn(meta_state['target_params'], ...) ??

aux_pi_target  = ??

agent_loss = π_loss + y_loss + z_loss + aux_pi_loss + q_loss.
...
问: 'target/EMA copy of the agent network' 是个什么东西？是一个NN吗？在哪里定义的？NN的结构是怎样的？input/output是什么？作用是什么？ 思路是什么？

答案：不一定正确？？
target_q_value，是一个prediction，来自 `agent_network + meta_state['target_params']`。

问：meta_state['target_params'] 又是个什么东西呢 ？ 是指已经与 target 对齐的agent params 吗？

相关代码和解释如下：
# Unroll the target policy.
target_q_net_out, _ = unroll_policy_fn(meta_state['target_params'], ...)
target_q_net_out is predictions from a target/EMA copy of the agent network (a target policy/value head), used for bootstrapping. 
In DiscoRL, the target outputs are computed by unrolling the agent net with meta_state['target_params'].
...
```text
inputs: 
  π, meta: π_hat, agent: π,  loss: KL(π_hat, π);
  y, meta: y_hat, agent: y, loss: KL(y_hat, y);
  z, meta: z_hat, agent: z, loss: KL(z_hat, z);
  aux_pi, meta: ??, agent: aux_pi, loss: ??;
  q, meta: ??, agent: q_v, loss: ??.

training: 
  agent_loss = KL(π_hat, π) + KL(y_hat, y) + KL(z_hat, z) + loss_aux_pi(??) + loss_q(??); 
  gradients <- agent_loss ;
  new agent_params <- gradients.
...
```text
inputs:
  observations, from actor-env;
  targets, from meta-network;
  aux_pi, q_v ? 

inference: 
  predictions, rollouts from agent-network;
  actions, selected from agent_outs['logits'];
  interactes with env, actor excute actions selected with env;

outputs:
  actor_rollouts, from actor vs env.
```
...
inputs:
  agent_outputs,
  aux_pi, from ?
  q_v, from ValueFunction.

training:
  meta_loss = ?;
  gradients <- meta_loss;
  new meta_params < update, apply_gradients;
...
```python
 ...
 172	    (value_loss, ...), dv_dparams = jax.value_and_grad(value_loss_fn, has_aux=True)(...)
            # dv_dparams = delta(value) / delta(params), 这里的 params 指 ValueFunc network的params.
 ...
...
3) **Meta-training (outer-loop)**
   ...
   - Inner-loop:
     - the above agent update runs with `is_meta_training=True` so targets are differentiable.  

   - Outer-loop:
     - evaluate adapted agent;   
     - compute outer policy-gradient loss using a separate `ValueFunction` to get advantages;   
     - take gradient w.r.t. update rule params; and   
     - update meta-network weights.  
...
3) **Value function** (meta-training only) consumes:  
   ...
   - and produces advantages/TDs for the *outer* meta-loss.   
   - It is trained online to track returns for the changing policy distribution.  
...
- Agent network ↔ meta-network wiring:   
`Agent.learner_step` → `DiscoUpdateRule.unroll_meta_net` → meta targets   
(see `agent.py:275-286` and `disco.py:132-180`).  
...
`q_loss` is not missing;   
it is computed in a separate function `agent_loss_no_meta(...)`   
and then added to the total loss in `Agent._loss(...)`.  
...
- Computes corrected returns and a policy-gradient advantage under off-policy data using importance weights `rho`.  问：这句完全不懂 ？ 

- Inputs:   
target V(s_t), target V(s_{t+1}), rewards, discounts, rho, lambda.  

- Outputs (in this code):   
`value_target`, `td`, `adv = pg_advantage`.  
问: value_target 是什么 ？作用是什么？ 数据的内容是什么？
...
问: 这里的 'trace coefficients'是指对未来的预测值打一点折扣，然后和当下的值汇总求和，作为 Q_value 的值。这有点像现金折现的算法，越远的未来收入折扣越大，越近的未来收入折扣越小，而今年的收入没有折扣。原因是，越远的未来，其预测值越不靠谱，但又不能完全忽略，所以打个折扣折衷一下。就像一个公司经理展望未来的辉煌，不能不信，也不能全信，所以打个折扣折衷一下。这种对未来打折扣的方法，虽然并不精确，却避开了最坏的结果，是一种无可奈何的务实和妥协。

- Inputs:   
Q(s_t, a_t) (chosen action), V(s_t), rewards, discounts, `c_t = lambda * min(rho, 1)`.  

- Outputs:   
`q_target`, `q_td`, `adv = q_target - V`.  

问: 这里的 q_target, q_td, adv，分别是什么含义？作用是什么？数据的格式是怎样的？  
...
问: 这里的 'target net', 'EMA net' 是个什么东西？是一种单独的NN吗？是2个东西还是1个东西？  
bootstrapping 是怎么实现的 ？  
trace 是怎么实现的 ？  
...
问：这段完全看不懂 ？
value_target 是个怎样的存在 ？
value_target 与 batch_vtrace_fn() 是怎么个关系？
...
问: 是否指actor所使用的那个selected π ? 用于在env中真正执行并产生数据的π ?

- **target/current policy** π(a|s): the policy you want to evaluate/update.  
问: 是否指 agent 生成的prediction中的π ?
...
`normalized_adv` is advantage scaled/centered using running (EMA) estimates of mean and variance.   

`normalized_adv` 是一个advantage值。该advantage值经过EMA方法进行了缩放调整(scaled/centered)，在EMA处理时用到了'均值(mean)'和'方差(variance)'。
'runing estimates'指EMA方法，因为EMA方法给出的是一个近似的估计值estimate。

公式：  

- normalized_adv = (score - M_ema) / sqrt(V_ema)  
- score，指agent在game中获得的原始得分。
- M_ema，经EMA处理过的均值mean：    new_M = α * old_M + (1 - α) * new_score
- V_ema，经EMA处理过的方差variance：new_V = α * old_V + (1 - α) * new_score
- α = EMA衰减系数，越接近1记忆力越强，RL中常设为0.99  
- M = mean     = 均值 = sum(scores) / n  
- V = variance = 方差 = sum((score - M)^2) / n
- n = 该批次数据的个数
- sqrt(V) = 标准差 

normalized_adv，一句话解释，就是把剧烈波动的原始score得分，转换为一种波动平滑、且均值在0附近的一种新数据。有点像电路板中的稳压器或滤波器，把毛刺去掉，能给后续提供稳定电流，避免把器件烧坏。  

因为游戏的不同，或者选手水平的波动，这种原始得分的绝对值波动会非常大，若直接用来训练模型会导致梯度爆炸进而训练崩溃；或者模型有重要进步时，在其它超大数值的遮挡下被忽略。  

为了解决这些问题，有人发明了 normalized_advantage，把 mean, variance, EMA 这几种方法组合起来，让不同游戏、不同选手、不同场次的得分能放在一张图表中比较分析，使得normalized_adv的均值在0上下浮动，方差在[-1, +1]附近波动。这样就给后续的模型训练提供了稳定、平滑、又保持灵敏的训练数据。
...
It’s modified by gradient descent on a value loss computed from TD targets (V-trace style),   
and its EMA normalization statistics are updated each step.   

It is continuously modified because:  
